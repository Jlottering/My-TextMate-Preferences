...
$PREFS_FILE = (empty($PROJECT_DIR) ? $_ENV['TM_DIRECTORY'] : $PROJECT_DIR).'/.ftpssh_settings';
$PREFS = array();
...
function get_file($TM_FILENAME, $TM_FILEPATH, $TM_DIRECTORY, $PROJECT_DIR, $PREFS) 
{
	...
	// Upload sucessful
	notify('Reloaded "'.$_ENV['TM_FILENAME'].'"');
...
function put_file($TM_FILENAME, $TM_FILEPATH, $TM_DIRECTORY, $PROJECT_DIR, $PREFS) {
	...
	// Upload sucessful
	notify('Uploaded "'.$_ENV['TM_FILENAME'].'"');
  return;
}
...
function settings_dialog($PREFS_FILE, $prefs='') {
	...
	$result = shell_exec($_ENV['DIALOG']." -cmp '".$default."' '".$_ENV['TM_BUNDLE_SUPPORT']."/nibs/FTP_SSH Settings.nib'");
